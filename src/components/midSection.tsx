...
export default function MidSection({ folder }: MidSectionProps) {
  const [allNotes, setNotes] = useState<note[]>([]);

  const getNotes = async () => {
    await axios
      .get<notes>(`api/notes?folderId=${folder.id}&page${1}&limit=${10}`)
      .then((res) => {
        const data = res.data;
        setNotes([...data.notes]);
      })
  ...
  return (
    <div className="bg-dark-gray flex flex-col gap-y-4 w-[20%]">
